...
self.onmessage = (e: MessageEvent<InMessage>) => {
  ...
  if (msg.type === "cancel") {
    ...
    generate(msg).catch((err: unknown) => {
      self.postMessage({ type: "error", taskId: msg.taskId, error: String(err) });
    });
  }
};
...
async function generate(msg: GenerateMessage): Promise<void> {
  ...
  for (let i = 0; i < total; i++) {
    ...
    lines.push(`; ── Object ${i + 1} (${obj.id.slice(0, 8)}) ─────────────────`);
    ...
    lines.push("");
    self.postMessage({ type: "progress", taskId, percent: Math.round(((i + 1) / total) * 100) });
    await sleep(0);
  }
...
type Subpath = Pt[];
interface PathToken { type: string; args: number[]; }

// ── Coordinate transform ──────────────────────────────────────────────────────

function transformPt(obj: VectorObject, config: MachineConfig, svgX: number, svgY: number): Pt {
  ...
  x += obj.x;
  y += obj.y;
  if (config.origin === "bottom-left") y = config.bedHeight - y;
  return { x, y };
}

// ── Main flattener ────────────────────────────────────────────────────────────

function flattenToSubpaths(obj: VectorObject, config: MachineConfig): Subpath[] {
  ...
  for (const cmd of abs) {
    ...
    switch (t) {
      ...
      case "Z":
        push(startX, startY);
        subpaths.push(cur); cur = [];
        lastCpX = cx; lastCpY = cy; break;
      case "C":
        for (let i = 0; i < a.length; i += 6) {
          for (const p of cubicBezier(cx, cy, a[i], a[i+1], a[i+2], a[i+3], a[i+4], a[i+5])) push(p.x, p.y);
          lastCpX = a[i+2]; lastCpY = a[i+3];
        }
        break;
      case "S":
        for (let i = 0; i < a.length; i += 4) {
          const c1x = 2*cx - lastCpX, c1y = 2*cy - lastCpY;
          for (const p of cubicBezier(cx, cy, c1x, c1y, a[i], a[i+1], a[i+2], a[i+3])) push(p.x, p.y);
          lastCpX = a[i]; lastCpY = a[i+1];
        }
        break;
      case "Q":
        for (let i = 0; i < a.length; i += 4) {
          for (const p of quadBezier(cx, cy, a[i], a[i+1], a[i+2], a[i+3])) push(p.x, p.y);
          lastCpX = a[i]; lastCpY = a[i+1];
        }
        break;
      case "T":
        for (let i = 0; i < a.length; i += 2) {
          const cpx = 2*cx - lastCpX, cpy = 2*cy - lastCpY;
          for (const p of quadBezier(cx, cy, cpx, cpy, a[i], a[i+1])) push(p.x, p.y);
          lastCpX = cpx; lastCpY = cpy;
        }
        break;
      case "A":
        for (let i = 0; i < a.length; i += 7) {
          for (const p of arcToBeziers(cx, cy, a[i], a[i+1], a[i+2], a[i+3], a[i+4], a[i+5], a[i+6])) push(p.x, p.y);
          lastCpX = cx; lastCpY = cy;
        }
        break;
    }
...
function tokenizePath(d: string): PathToken[] {
  ...
  while ((m = re.exec(d)) !== null) {
    const args = m[2].trim()
      ? m[2].trim().split(/[\s,]+|(?=-)/).filter(Boolean).map(Number).filter(isFinite)
      : [];
  ...
  for (const tok of tokens) {
    ...
    switch (T) {
      case "M": {
        ...
        abs.push({ type: "M", args: out }); break;
      }
      case "L": {
        const out: number[] = [];
        for (let i = 0; i < a.length; i += 2) {
          cx = rel ? cx + a[i] : a[i]; cy = rel ? cy + a[i+1] : a[i+1];
          out.push(cx, cy);
        }
        abs.push({ type: "L", args: out }); break;
      }
      case "H": {
        const out: number[] = [];
        for (let i = 0; i < a.length; i++) { cx = rel ? cx + a[i] : a[i]; out.push(cx); }
        abs.push({ type: "H", args: out }); break;
      }
      case "V": {
        const out: number[] = [];
        for (let i = 0; i < a.length; i++) { cy = rel ? cy + a[i] : a[i]; out.push(cy); }
        abs.push({ type: "V", args: out }); break;
      }
      case "C": {
        const out: number[] = [];
        for (let i = 0; i < a.length; i += 6) {
          out.push(
            rel ? cx+a[i]   : a[i],   rel ? cy+a[i+1] : a[i+1],
            rel ? cx+a[i+2] : a[i+2], rel ? cy+a[i+3] : a[i+3],
            rel ? cx+a[i+4] : a[i+4], rel ? cy+a[i+5] : a[i+5]
          );
          cx = out[out.length-2]; cy = out[out.length-1];
        }
        abs.push({ type: "C", args: out }); break;
      }
      case "S": {
        const out: number[] = [];
        for (let i = 0; i < a.length; i += 4) {
        ...
        abs.push({ type: "S", args: out }); break;
      }
      case "Q": {
        const out: number[] = [];
        for (let i = 0; i < a.length; i += 4) {
        ...
        abs.push({ type: "Q", args: out }); break;
      }
      case "T": {
        const out: number[] = [];
        for (let i = 0; i < a.length; i += 2) {
        ...
        abs.push({ type: "T", args: out }); break;
      }
      case "A": {
        const out: number[] = [];
        for (let i = 0; i < a.length; i += 7) {
          const ex = rel ? cx+a[i+5] : a[i+5], ey = rel ? cy+a[i+6] : a[i+6];
          out.push(a[i], a[i+1], a[i+2], a[i+3], a[i+4], ex, ey);
          cx = ex; cy = ey;
        }
        abs.push({ type: "A", args: out }); break;
      }
    }
  }
  return abs;
...
function cubicBezier(x0: number, y0: number, x1: number, y1: number, x2: number, y2: number, x3: number, y3: number): Pt[] {
  const pts: Pt[] = [];
...
function subdivideCubic(x0: number, y0: number, x1: number, y1: number, x2: number, y2: number, x3: number, y3: number, out: Pt[]): void {
  const ux = 3*x1 - 2*x0 - x3, uy = 3*y1 - 2*y0 - y3;
  const vx = 3*x2 - 2*x3 - x0, vy = 3*y2 - 2*y3 - y0;
  if (Math.max(ux*ux + uy*uy, vx*vx + vy*vy) <= FLATNESS_SQ * 16) return;
...
function quadBezier(x0: number, y0: number, cpx: number, cpy: number, x1: number, y1: number): Pt[] {
  const c1x = x0 + 2/3*(cpx-x0), c1y = y0 + 2/3*(cpy-y0);
  const c2x = x1 + 2/3*(cpx-x1), c2y = y1 + 2/3*(cpy-y1);
  return cubicBezier(x0, y0, c1x, c1y, c2x, c2y, x1, y1);
}

// ── SVG arc to cubics (SVG spec algorithm) ────────────────────────────────────

function arcToBeziers(x1: number, y1: number, rx: number, ry: number, xRot: number, largeArc: number, sweep: number, x2: number, y2: number): Pt[] {
  ...
  const ang = (ux: number, uy: number, vx: number, vy: number) => {
    ...
    return (ux*vy - uy*vx < 0 ? -1 : 1) * Math.acos(Math.min(1, Math.max(-1, (ux*vx+uy*vy)/n)));
  };
