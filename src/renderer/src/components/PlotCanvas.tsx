...
export function PlotCanvas() {
  ...
  const setFitted            = useCallback((v: boolean) => { fittedRef.current = v; _setFitted(v); }, []);

  // ── Fit helpers ───────────────────────────────────────────────────────────────
  const computeFit = useCallback((w: number, h: number): Vp => {
    const pad  = 32;
    const zoom = Math.max(MIN_ZOOM, Math.min((w - pad * 2) / canvasW, (h - pad * 2) / canvasH));
    return { zoom, panX: (w - canvasW * zoom) / 2, panY: (h - canvasH * zoom) / 2 };
  }, [canvasW, canvasH]);
  ...
  const zoomBy = useCallback((factor: number, clientX?: number, clientY?: number) => {
    ...
    setVp({ zoom: newZoom, panX: mx - cx * newZoom, panY: my - cy * newZoom });
    setFitted(false);
  }, [setVp, setFitted]);
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver((entries) => {
      ...
      if (first || fittedRef.current) {
        ...
        setVp({ zoom: old.zoom, panX: width / 2 - cx * old.zoom, panY: height / 2 - cy * old.zoom });
      }
    });
    ro.observe(el);
  ...
  // ── Pan state ─────────────────────────────────────────────────────────────────
  const panStartRef = useRef<{ mx: number; my: number; panX: number; panY: number } | null>(null);
  const [spaceDown, setSpaceDown] = useState(false);
  const spaceRef    = useRef(false);
  const [isPanning, setIsPanning] = useState(false);

  const startPan = useCallback((clientX: number, clientY: number) => {
    panStartRef.current = { mx: clientX, my: clientY, panX: vpRef.current.panX, panY: vpRef.current.panY };
    setIsPanning(true);
  }, []);
  ...
  const [scaling, setScaling] = useState<{
    id: string; handle: HandlePos;
    startMouseX: number; startMouseY: number;
    startScale: number;  startObjX: number; startObjY: number;
  ...
  }, [selectedImportId, toolpathSelected, removeImport, selectImport,
      setGcodeToolpath, zoomBy, fitToView]);
  ...
  const onImportMouseDown = useCallback(
    (e: React.MouseEvent, id: string) => {
      ...
      if (!imp) return;
      setDragging({ id, startMouseX: e.clientX, startMouseY: e.clientY, startObjX: imp.x, startObjY: imp.y });
    },
    [selectImport],
  );
  ...
  const onHandleMouseDown = useCallback(
    (e: React.MouseEvent<SVGCircleElement>, id: string, handle: HandlePos) => {
      ...
      setScaling({
        ...
        startObjX:   imp.x, startObjY: imp.y,
  ...
  const onMouseMove = useCallback(
    (e: MouseEvent) => {
      ...
      if (panStartRef.current) {
        ...
        setVp({ zoom: vpRef.current.zoom, panX: panStartRef.current.panX + dx, panY: panStartRef.current.panY + dy });
        setFitted(false);
        return;
      }
  ...
  const onZoomIn  = useCallback((e: React.MouseEvent) => { e.stopPropagation(); zoomBy(ZOOM_STEP); },         [zoomBy]);
  const onZoomOut = useCallback((e: React.MouseEvent) => { e.stopPropagation(); zoomBy(1 / ZOOM_STEP); },     [zoomBy]);
  const onFit     = useCallback((e: React.MouseEvent) => { e.stopPropagation(); fitToView(); },               [fitToView]);

  // ── Cursor ────────────────────────────────────────────────────────────────────
  const cursor = spaceDown ? (isPanning ? "grabbing" : "grab") : isPanning ? "grabbing" : undefined;
  ...
  return (
    ...
    >
              ...
              <>
                ...
                {toolpathSelected && (
                  <g pointerEvents="none">
                    <rect x={svgLeft} y={svgTop} width={bboxW} height={bboxH}
                      fill="none" stroke="#38bdf8" strokeWidth={1} strokeDasharray="5 3" />
                    {([[svgLeft, svgTop], [svgRight, svgTop], [svgLeft, svgBottom], [svgRight, svgBottom]] as [number, number][])
                      .map(([cx, cy], i) => (
                ...
                {toolpathSelected && (
                  <g style={{ cursor: "pointer" }}
                    ...
                    <text x={delCx} y={delCy} textAnchor="middle" dominantBaseline="central"
                      fill="white" fontSize={11} fontWeight="bold" style={{ userSelect: "none" }}>×</text>
                  </g>
                )}
              </>
            );
      ...
      <div className="absolute bottom-4 right-4 flex flex-col gap-1 z-10" onMouseDown={(e) => e.stopPropagation()}>
        <button
  ...
  return (
    <g>
      ...
      >
        ...
        <rect x={vbX} y={vbY} width={imp.svgWidth} height={imp.svgHeight} fill="transparent" />
      </g>
      ...
      {selected && (
        <g style={{ cursor: "pointer" }} onClick={(e) => { e.stopPropagation(); onDelete(); }}>
          ...
          <text x={delCx} y={delCy} textAnchor="middle" dominantBaseline="central"
            fill="white" fontSize={12} fontWeight="bold" style={{ userSelect: "none" }}>×</text>
        </g>
      )}
    </g>
  );
