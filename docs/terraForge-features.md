...
- [x] **G-code generation options dialog** — clicking "Generate G-code" opens a tabbed modal with **Paths**, **Options**, **Output**, and (when enabled) **Vinyl** tabs; supports optimisation, join tolerance, output destination selection, page clipping, custom start/end G-code, and split-output modes for per-SVG-layer, per-layer-group, and per-colour-group export; the split-output modes are controlled by a master checkbox with mutually exclusive radio choices, and hatch-only colour exports are nested under colour-group mode; at least one output destination must be selected (Generate button is disabled otherwise)
- [x] **Persisted generation preferences** — dialog settings are stored in `localStorage` under `terraforge.gcodePrefs` and restored on the next session, including join tolerance, delay/speed overrides, split-output mode, hatch export choice, output choices, and vinyl options (weed border enable + margin)
- [x] **(EXPERIMENTAL) Vinyl cutting options** — dedicated **Vinyl** tab in Generate G-code dialog includes: drag-knife compensation toggle, weed border toggle, and per-job weed border margin; the tab is shown only when app-level vinyl features are enabled
- [x] **(EXPERIMENTAL) Vinyl compensation defaults in app config** — Application Configuration exposes drag-knife defaults used by generation: blade offset, corner angle threshold, and blade rotation offset
- [x] **(EXPERIMENTAL) Ink/brush service moves** — travel-triggered pen/brush maintenance: configurable service stations (prime, wipe, dip, wash) with optional motion recipes; two modes: **prime-wipe** (press station for ink priming followed by wipe station) and **brush-dip** (cyclic dip stations with optional wash cadence); stations are visited when accumulated rapid-travel distance exceeds a configurable threshold (with optional jitter); each station can perform a scripted motion (prime-press with configurable depth and repetitions, or brush-motion with back-forth or circular patterns); wash cadence in brush-dip mode applies wash station after every Nth dip; service moves are emitted as G0 plunges interleaved into the G-code output; settings are stored per G-code generation job and reported in the G-code header comment; layer-to-station mapping allows assigning SVG layers to specific dip stations
...
- [x] **Path direction strategy** — when "Optimise paths" is enabled, a "Path direction" radio group allows users to choose between two modes: (1) "Minimize travel by reversing paths" (default) — optimizer evaluates both the start and end points of each candidate path, reversing it if the endpoint is closer to the next path's start, significantly reducing travel for hatching and parallel-line patterns; (2) "Respect source path direction" — disables path reversal, preserving original directions; useful when backtrack-mode passes are active and forward/reverse pairing must be maintained; both modes reported in G-code header comment
...
- [x] **Plot points (circles)** — optional import-level toggle that emits a single pen tap at the geometric centre of each source circle, intended for stippling workflows where circles represent dots; this is additive to normal circle processing, so stroke outlines and fill/hatch output are still generated when enabled
