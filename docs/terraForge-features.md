...
- [x] **Persisted generation preferences** — all four dialog options (including join paths and tolerance) are stored in `localStorage` under `terraforge.gcodePrefs` and restored on the next session; defaults are Optimise=on, Join paths=off (tolerance 0.2 mm), Upload to SD=on, Save to computer=off
