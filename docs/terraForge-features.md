...
- [x] **G-code generation options dialog** — clicking "Generate G-code" opens a tabbed modal with **Paths**, **Options**, **Output**, and (when enabled) **Vinyl** tabs; supports optimisation, join tolerance, output destination selection, page clipping, custom start/end G-code, and export split modes; at least one output destination must be selected (Generate button is disabled otherwise)
- [x] **Persisted generation preferences** — dialog settings are stored in `localStorage` under `terraforge.gcodePrefs` and restored on the next session, including join tolerance, delay/speed overrides, output choices, and vinyl options (weed border enable + margin)
