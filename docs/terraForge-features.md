...
- [x] **G-code generation options dialog** — clicking "Generate G-code" opens a modal with four independently-toggled options before generation begins: **Optimise paths** (nearest-neighbour reorder), **Join nearby paths** (experimental — merges path endpoints within tolerance), **Upload to SD card** (direct upload to machine), and **Save to computer** (native save dialog); any combination is valid; both upload and save can be active simultaneously; at least one output must be selected (Generate button is disabled otherwise)
- [x] **Persisted generation preferences** — all dialog options (including join paths and tolerance, lift pen, return to home, and custom start/end G-code) are stored in `localStorage` under `terraforge.gcodePrefs` and restored on the next session; defaults are Optimise=on, Join paths=off (tolerance 0.2 mm), Upload to SD=on, Save to computer=off, Lift pen at end=on, Return to home=off, custom G-code fields empty
- [x] **Custom start / end G-code** — "Generate G-code" dialog contains two monospaced text areas: one for G-code inserted after the preamble (before paths), one appended after the lift/return commands; content is persisted in preferences
- [x] **Lift pen at end** — checkbox in the generation dialog (default: on); when enabled, sends `penUpCommand` after the last stroke so the pen is not left pressed on the paper
- [x] **Return to home** — checkbox in the generation dialog (default: off); when enabled, appends `G0 X0 Y0` after the pen-lift (if any) at end of job
