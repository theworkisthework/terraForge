...
- [x] **Vinyl cutting options** — dedicated **Vinyl** tab in Generate G-code dialog includes: drag-knife compensation toggle, weed border toggle, and per-job weed border margin; the tab is shown only when app-level vinyl features are enabled
- [x] **Vinyl compensation defaults in app config** — Application Configuration exposes drag-knife defaults used by generation: blade offset, corner angle threshold, and blade rotation offset
